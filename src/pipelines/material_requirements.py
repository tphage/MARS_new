...
def run_fixed_pipeline(
    ...
    **kwargs
    ...
    7. Find connections in KG → ResearchScientist
    ...
    Args:
        ...
        scientist: ResearchScientist instance (required, for Step 7 - Material Properties KG)
    ...
    Returns:
        Dict containing:
            ...
            - "keyword_connections": Dict with connections between keywords in knowledge graph (if ResearchScientist available)
            ...
            - "connections_found": Boolean indicating if connections were found in knowledge graph
    """
    print("=" * 70)
    ...
    # Step 7: Find connections between keywords
    keyword_connections = None
    connections_found = False
    if extracted_keywords and len(extracted_keywords) >= 2:
        print("\n" + "-" * 70)
        print("Step 7: ResearchScientist - Finding Connections Between Keywords")
        print("-" * 70)
        print(f"→ Finding connections between {len(extracted_keywords)} keywords in knowledge graph...")
        try:
            keyword_connections = scientist.find_connections(
                keywords=extracted_keywords,
                use_best_match_only=True
            )
            
            summary = keyword_connections.get('summary', {})
            strategy = summary.get('strategy', 'single')
            
            if summary.get('connections_found'):
                ...
                if strategy == 'separate':
                    ...
                    print(f"✓ Connections found across both knowledge graphs:")
                    if kg1_info:
                        kg1_summary = kg1_info.get('summary', {})
                        kg1_name = kg1_info.get('name', 'KG1')
                        if kg1_summary.get('connections_found'):
                            print(f"  - {kg1_name}: {kg1_summary.get('num_paths_found', 0)} paths, "
                                  f"{kg1_summary.get('subgraph_nodes', 0)} nodes, {kg1_summary.get('subgraph_edges', 0)} edges")
                        else:
                            print(f"  - {kg1_name}: No connections found")
                    
                    if kg2_info:
                        kg2_summary = kg2_info.get('summary', {})
                        kg2_name = kg2_info.get('name', 'KG2')
                        if kg2_summary.get('connections_found'):
                            print(f"  - {kg2_name}: {kg2_summary.get('num_paths_found', 0)} paths, "
                                  f"{kg2_summary.get('subgraph_nodes', 0)} nodes, {kg2_summary.get('subgraph_edges', 0)} edges")
                        else:
                            print(f"  - {kg2_name}: No connections found")
                    
                    print(f"  Total: {summary.get('subgraph_nodes', 0)} nodes, {summary.get('subgraph_edges', 0)} edges")
                else:
                    # Merged or single KG strategy
                    print(f"✓ Connections found: {summary.get('subgraph_nodes', 0)} nodes, {summary.get('subgraph_edges', 0)} edges")
                
                print(f"  Algorithm used: {summary.get('algorithm_used', 'unknown')}")
            else:
                print(f"✗ No connections found between keywords")
                if 'error' in summary:
                    print(f"  Error: {summary['error']}")
                elif strategy == 'separate':
                    # Show which KGs were queried even if no connections found
                    kg1_info = summary.get('kg1', {})
                    kg2_info = summary.get('kg2', {})
                    if kg1_info or kg2_info:
                        print(f"  Queried knowledge graphs: {kg1_info.get('name', 'KG1') if kg1_info else ''}, {kg2_info.get('name', 'KG2') if kg2_info else ''}")
        except Exception as e:
            print(f"✗ Error finding connections: {str(e)}")
            keyword_connections = {
                "error": str(e),
                "summary": {"connections_found": False}
            }
    elif not extracted_keywords or len(extracted_keywords) < 2:
        print("\n" + "-" * 70)
        print("Step 7: ResearchScientist - Skipped (insufficient keywords)")
        print("-" * 70)
        if not extracted_keywords:
            print("  → No keywords extracted, skipping connection finding")
        elif len(extracted_keywords) < 2:
            print(f"  → Only {len(extracted_keywords)} keyword(s) available, need at least 2 for connections")
    ...
    if keyword_connections:
        ...
        if summary.get('connections_found'):
            if strategy == 'separate':
                kg1_info = summary.get('kg1', {})
                kg2_info = summary.get('kg2', {})
                kg1_paths = kg1_info.get('summary', {}).get('num_paths_found', 0) if kg1_info else 0
                kg2_paths = kg2_info.get('summary', {}).get('num_paths_found', 0) if kg2_info else 0
                print(f"  - Keyword connections: {summary.get('subgraph_nodes', 0)} nodes, {summary.get('subgraph_edges', 0)} edges "
                      f"({kg1_info.get('name', 'KG1') if kg1_info else ''}: {kg1_paths} paths, "
                      f"{kg2_info.get('name', 'KG2') if kg2_info else ''}: {kg2_paths} paths)")
            else:
                print(f"  - Keyword connections: {summary.get('subgraph_nodes', 0)} nodes, {summary.get('subgraph_edges', 0)} edges")
    print("=" * 70 + "\n")
